  ...
  $.fn.imageMaker = function (options) {
    ...
    var initialElements = function (canvas_info) {
      ...
      $.each(canvas_info.templates, function (delta, img) {
        ...
        buildImagesMemes +=
          '<img typeof="foaf:Image" width="' +
          canvas_info.template_thumbnail_width +
          '" height="' +
          canvas_info.template_thumbnail_height +
          '" src="' +
          img.url +
          '" title="' +
          img.title +
      ...
      var buildImagesMergeMemes =
        '<div class="wrapper-amm-select-merge-images">';
      buildImagesMergeMemes +=
        '<div class="wrapper-amm-hover-title-merge-images"></div>';
      ...
      $.each(canvas_info.merge_images, function (delta, img) {
        ...
        buildImagesMergeMemes +=
          '<img typeof="foaf:Image" width="' +
          canvas_info.merge_image_thumbnail_width +
          '" height="' +
          canvas_info.merge_image_thumbnail_height +
          '" src="' +
          img.url +
          '" title="' +
          img.title +
    ...
    <div class="wrapper-memes-preview-operations">
        ...
        `</span>
                ...
                <input type="file" class="upload" id="amm-input-upload-image-meme-responsive" name="addImage-responsive">             <span class="glyphicon glyphicon-paperclip"></span>             <span class="text_value">` +
        canvas_info.i18n.addTemplateText +
        `</span>
            </button>      
          ...
          <div class="wrapper-amm-stop-brushes">
           ...
           <button type="submit" class="btn btn-default" value="Stop Brushing" id="amm-stop-brushing">` +
        canvas_info.i18n.stopBrushingText +
        `</button>
    ...
    <canvas id="` +
        canvas_info.id +
        `" class="amm_canvas"></canvas>	   
    </div>
    <div class="canvas_loading">` +
        canvas_info.i18n.canvasLoadingText +
        `</div>
    
    </div> 
     
    <form class="form-generat-meme" action="meme-generator-callback" method="post" enctype="multipart/form-data">
    <a href="#" type="submit" class="generate_meme btn btn-default btn-primary form-submit" disabled="disabled" >` +
        canvas_info.i18n.imageGeneratorText +
        `</a>
    <button type="submit" class="btn btn-default btn-danger form-submit reset_meme" >` +
        canvas_info.i18n.resetText +
        `</button>
    </form>
    ...
    $(this).each(function (delta) {
      ...
      var canvas_info = {
        ...
        i18n: {
          ...
          resetText: "Reset",
          imageGeneratorText: "Download Bangla Poster",
          stopBrushingText: "Stop Brushing",
          canvasLoadingText: "Canvas Loading",
        },
        weight:
          "brushes" /*Order of Draw the Possibilities('brushes', 'text', 'images')*/,
      };
      ...
      var ImageInfo = function () {
        ...
        this.ratioLeft =
          this.left / container.find(selectors.ammCanvas).width();
      ...
      container.on("click", ".add_textbox", function (event) {
        ...
        if (delta == 1) {
          ...
          top =
            container.find(selectors.ammCanvas).height() -
            container.find(selectors.ammCanvas).outerHeight() *
              text_info.ratioHeight;

          //                console.log(top);
        }
        if (delta > 1) {
          top =
            container.find(selectors.ammCanvas).height() / 2 -
            (container.find(selectors.ammCanvas).outerHeight() *
              text_info.ratioHeight) /
              2;
        }
        ...
        var boxDR = $(
          '<div id="' +
            ...
            '"' +
            style +
            ' font="20px Arial" class="boxDR"></div>'
        );
        var delete_box = $(
          '<button href="#" class="delete-box-amm delete-text-boxes"></button>'
        );
        boxDR.append(delete_box);
        delete_box.on("click tap touchstart", function () {
          container
            .find("#" + text_info.id)
            .closest(".wrapper-text")
            .remove();
          container
            .find("#" + text_info.id_box)
            .resizable("destroy")
            .draggable("destroy")
            .remove();
        ...
        container
          ...
          .draggable({
            ...
            drag: function (event, ui) {
              ...
              container.trigger({
                type: "CanvasPreChange",
                value: { ui: ui, delta: delta, this: $(this) },
                on: "dragText",
              });
              MemeGeneratorAPI.canvas_reset(canvas_info.img);
              MemeGeneratorAPI.drawBrushesPoints();
      ...
      container.on("click", ".btn", function (event) {
        ...
        if (!$(event.target).is(sel)) {
          if ($(this).hasClass("trigger-element"))
            container.find($(this).attr("target_element_triggered")).click();
        } else {
      ...
      $(window).resize(function (event) {
        ...
        if (event.target == window) {
          ...
          $.each(texts.keys, function (index, id) {
            if (texts[id] != null) {
              container
                .find("#" + texts[id].id_box)
                .width(
                  container.find(selectors.ammCanvas).outerWidth() *
                    texts[id].ratioWidth
                );
              container
                .find("#" + texts[id].id_box)
          ...
          $.each(images.keys, function (index, id) {
            ...
            container
              ...
              .width(
                container.find(selectors.ammCanvas).outerWidth() *
                  images[id].ratioWidth
              );
            container
              .find("#" + images[id].id)
              .height(
                container.find(selectors.ammCanvas).outerHeight() *
                  images[id].ratioHeight
              );
            container
              .find("#" + images[id].id)
              .css(
      ...
      function amm_add_image_load(image_info) {
        image_info.id =
          "imageBoxDR" + "_" + canvas_info.id + "_" + images.length;
        ...
        image_info.img.onload = function () {
          ...
          var imageBoxDR = $(
            '<div id="' + image_info.id + '" class="imageBoxDR"></div>'
          );
          var delete_box = $(
            '<button class="delete-box-amm delete-pr-merge-image"></button>'
          );
          ...
          container
            ...
            .draggable({
              ...
              drag: function (event, ui) {
                ...
                container.trigger({
                  type: "CanvasPreChange",
                  value: { ui: ui, delta: image_info.delta, this: $(this) },
                  on: "dragImage",
                });
                //    console.log(images);
                MemeGeneratorAPI.canvas_reset(canvas_info.img);
      ...
      container.on(
        ...
        function (event) {
          container
            .find(".wrapper-amm-hover-title-merge-images")
          ...
          if (
            /^([\w]+\:)?\/\//.test(canvas_info.img.src) &&
            canvas_info.img.src.indexOf(location.host) === -1
          ) {
      ...
      container.on("click", ".wrapper_select_image_amm", function (event) {
        ...
        container
          .find(".wrapper_hover_title_meme_amm")
          .text($(this).find("img").attr("title"));
      ...
      /*Upload meme image*/
      container
        .find("#amm-input-upload-image-meme-responsive")
        .change(function (e) {
          var reader = new FileReader();
          reader.onload = function (event) {
            canvas_info.img = new Image();
      ...
      container.on(
        "change",
        ".wrapper-memes-main .EventChange",
        function (event) {
          ...
          $.each($(this).attr("var").split(","), function (index, variable) {
            ...
            var id = MemeGenerator.get_text_id(
              $this.closest(".wrapper-text").attr("delta")
            );
      ...
      container.on("click", ".moreOption", function (event) {
        ...
        if ($(this).hasClass("opened")) {
          $(this)
            .closest(".wrapper-text")
            .find(".wrapperRenderPopupMoreOptions")
            .remove();
      ...
      container.on("CanvasPreChange", function (event) {
        if (event.on == "dragText" || event.on == "resizeText") {
          ...
          if (event.on == "resizeText") {
            texts[id].ratioWidth =
              event.value.ui.size.width /
              container.find(selectors.ammCanvas).width();
            texts[id].ratioHeight =
              event.value.ui.size.height /
              container.find(selectors.ammCanvas).height();
            //        console.log([ event.value.ui.size.height, container.find(selectors.ammCanvas).height()]);
          }
          texts[id].ratioTop =
            event.value.ui.position.top /
            container.find(selectors.ammCanvas).height();
          texts[id].ratioLeft =
            event.value.ui.position.left /
            container.find(selectors.ammCanvas).width();
      ...
      $(container).on(
        "click",
        "#amm-brushes, #amm-clear-brush, #amm-stop-brushing",
        function (event) {
          event.preventDefault();
          ...
          switch (this.id) {
            ...
            case "amm-stop-brushing":
              ...
              if (MemeGenerator.is_responsive()) {
                container
                  .find(".wrapper-memes-preview-operations .responsive-button")
                  .show();
              }
      ...
      function amm_brushing(event) {
        if (brush) {
          if (isMouseDown) {
            ...
            var ratio_width_responsive =
              canvas_info.final_width /
              container.find(selectors.ammCanvas).width();
            var ratio_height_responsive =
              canvas_info.final_height /
              container.find(selectors.ammCanvas).height();
      ...
      $(container).on("hover", ".wrapper_select_image_amm img", function (ev) {
        ...
        if (ev.type == "mouseleave") {
          container
            .find(".wrapper_hover_title_meme_amm")
            .text($(".SI_amm_selected img").attr("title"));
        }
      });
      ...
      container.find(".generate_meme").click(function (event) {
        ...
        if (canvas_info.downloadGeneratedImage) {
          var image = canvas_info.canvas
            .get(0)
            .toDataURL("image/png")
            .replace("image/png", "image/octet-stream");
          $(this).attr(
            "download",
            container.find(selectors.wrapperHoverTitleMemeAmm).text() + ".png"
          );
        ...
        formDataMemeGeneratorData.append(
          "all_texts",
          JSON.stringify(all_texts)
        );
        formDataMemeGeneratorData.append("textsIsEmpty", texts.empty);
        formDataMemeGeneratorData.append(
          "drwaIsEmpty",
          brushespoints.length ? false : true
        );
        formDataMemeGeneratorData.append(
          "imagesIsEmpty",
          images.length ? false : true
        );
      ...
      MemeGeneratorAPI.window_width = function () {
        ...
        return $(window).width() > 600
          ? 500
          : $(window).width() - paddingLeft - paddingRight;
      };

      MemeGeneratorAPI.wrapText = function (
        context,
        text,
        x,
        y,
        ...
        if (words.length == 1) {
          ...
          for (var n = 0; n < words.length; n++) {
            ...
            if (metrics.width > maxWidth) {
              ...
              while (
                (delta + 2) * text_info.fontSize + spaceLines * (delta + 2) >
                maxHeight
              ) {
          ...
          for (var n = 0; n < words.length; n++) {
            ...
            if (metrics.width > maxWidth) {
              top =
                y + text_info.fontSize * (delta + 1) + spaceLines * (delta + 1);
      ...
      MemeGeneratorAPI.skipWidth = function (
        context,
        textInfo,
        text,
        maxWidth,
      ...
      MemeGeneratorAPI.canvas_reset = function (img) {
        ...
        else canvas_info.ratio_image = canvas_info.ratio_height;
        ...
        container
          .find(selectors.ammCanvas + "," + selectors.wrapperCanvas)
          .attr("width", canvas_info.final_width)
          .attr("height", canvas_info.final_height);

        canvas_info.ctx.drawImage(
          img,
          0,
          0,
          img.width,
      ...
      MemeGeneratorAPI.buildFont = function (text_info) {
        ...
        return (
          text_info.fontStyle +
          " " +
          text_info.fontVariant +
          " " +
          text_info.fontWeight +
          " " +
          text_info.fontSize +
          "px " +
          text_info.fontFamily
        );
      };
        ...
        container
          ...
          .css({
            left:
              ...
              text_info.elementChanged.outerWidth() -
              $(".wrapperRenderPopupMoreOptions").outerWidth() +
              "px",
      ...
      MemeGeneratorTheme.wrapper_text = function (text_info) {
        return (
          ...
          `" id="` +
          text_info.id +
          `" class="amm-text"></textarea>
                <div class="fontOps">
                <input type="color" data-on="text" class="amm-color-picker pickerTextColor" title="Change Font Color"  name="pickerTextColor" value="` +
          text_info.textColor +
          `" list>
                <input type="color" data-on="outline" class="amm-color-picker pickerStrokeColor" title="Change Outline Color"  name="pickerStrokeColor" value="` +
          text_info.strokeColor +
          `" list>
                <input id="ow0" class="ow EventChange" var="lineWidth" value="` +
          text_info.lineWidth +
          `" type="number" maxlength="1" min="0" max="9" title="Change Outline Width" tabindex="-1">
      ...
      MemeGeneratorTheme.renderPopupMoreOptions = function (text_info) {
        var InputFontSize =
          '<div class="wrapperInputFontSize"><label>' +
          canvas_info.i18n.sizeText +
          ': </label><input class="InputFontSize EventChange" var="fontSize,inputFontSize" type="number" maxlength="1" max="100" min="10" title="Change Size Text" tabindex="-1" value="' +
          text_info.inputFontSize +
          '"></div>';
        var All_FontFamilies = [
          "Verdana",
          "impact",
          "Times New Roman",
          "Arial",
        ...
        $(All_FontFamilies).each(function (index, value) {
          if (text_info.fontFamily == value)
            HTML_FontFamilies +=
              '<option selected="selected">' + value + "</option>";
          else HTML_FontFamilies += "<option>" + value + "</option>";
        });
        var InputFontFamily =
          '<div class="wrapperInputFontFamily"><label>' +
          canvas_info.i18n.fontFamilyText +
          ': </label><select var="fontFamily" class="InputFontFamily EventChange" >' +
          HTML_FontFamilies +
          "</select></div>";
        checked = "";
        if (text_info.shadowBlur) checked = 'checked="checked"';
        var InputShadow =
          '<div class="wrapperInputFontFamily"><label>Shadow: </label><input class="InputShadow EventChange" var="shadowBlur" value="' +
          text_info.shadowBlur +
          '" type="checkbox" ' +
          checked +
          "></div>";
        var InputUpperCase =
          '<div class="wrapperInputUpperCase"><label>' +
          canvas_info.i18n.uperCaseText +
          ': </label><input class="InputUpperCase EventChange" var="toUpperCase" value="' +
          text_info.shadowBlur +
          '" type="checkbox" ' +
          (text_info.toUpperCase ? 'checked="checked"' : "") +
          "></div>";
        var ELEMENT =
          '<div class="wrapperRenderPopupMoreOptions amm-popup">' +
          InputFontFamily +
          InputFontSize +
          InputShadow +
          InputUpperCase +
      ...
      MemeGeneratorTheme.popupDownloadCanvas = function (args) {
        ...
        return (
          '<div class="wrapper-popup-download-canvas">\n\
        <img class="done-image-canvas" src="' +
          canvas_info.canvas.toDataURL() +
          '">\n\
